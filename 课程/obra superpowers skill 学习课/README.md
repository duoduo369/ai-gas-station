...
- [0002 · 功能做出来以后，工作流怎么真正收口](./lessons/0002-how-the-workflow-actually-finishes.html) - 讲后半程：`subagent-driven-development` 是什么、`executing-plans` 什么时候接、review 为什么是硬闸门、`finishing-a-development-branch` 怎么收口。
- [0003 · 实现跑偏时，补任务、改计划，还是回到 spec](./lessons/0003-when-implementation-drifts-from-spec.html) - 讲 agent 报 done 以后还被 reviewer 或人验收抓出跑偏时，怎么区分实现缺口、plan 缺口、spec 缺口。
- [0004 · 人验收发现 bug 后怎么切线，以及 worktree 到底在流程里的哪一层](./lessons/0004-qa-bug-flow-and-where-worktrees-fit.html) - 讲验收 bug 时怎么切到 `systematic-debugging -> verification-before-completion`，也讲 `using-git-worktrees` 为什么一直待在实现 / 修 bug 的环境层。
