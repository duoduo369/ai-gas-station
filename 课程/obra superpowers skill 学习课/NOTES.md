...
- 资源引用改成本课自包含：`course.css` 与 `open-links-new-tab.js` 已复制到本课 `assets/`，所有页面统一用 `../assets/` 路径（不再依赖 `../../assets/` 共享位置）。封面图复制自 `../../assets/cover/Superpowers课程.jpg`，放在本课 `assets/cover/cover.jpg`。
