# teach skill Resources

## Knowledge

- [teach SKILL.md](https://github.com/mattpocock/skills/blob/main/skills/productivity/teach/SKILL.md)
  agent 真正执行的规则卡。Use for: 工作区文件、知识/技能/智慧、流畅 vs 存储、使命与最近发展区。
- [docs/productivity/teach.md](https://github.com/mattpocock/skills/blob/main/docs/productivity/teach.md)
  给人读的说明。Use for: 何时伸手、和 wait-what / grill-me / research 的分工、常见坑、怎样算它在工作。
- [teach on skills.sh](https://www.skills.sh/mattpocock/skills/teach)
  安装入口与 skill 正文镜像。Use for: `npx skills add` 以及和仓库对照。
- [MISSION-FORMAT.md](https://github.com/mattpocock/skills/blob/main/skills/productivity/teach/MISSION-FORMAT.md)
  使命文件格式。Use for: Why / Success / Constraints / Out of scope 怎么写短。
- [RESOURCES-FORMAT.md](https://github.com/mattpocock/skills/blob/main/skills/productivity/teach/RESOURCES-FORMAT.md)
  资料清单格式。Use for: Knowledge / Wisdom 分组，以及为什么每条都要写“何时用”。
- [LEARNING-RECORD-FORMAT.md](https://github.com/mattpocock/skills/blob/main/skills/productivity/teach/LEARNING-RECORD-FORMAT.md)
  学习记录格式。Use for: 什么才算“学会了”、什么只是“讲过了”。
- [playbook/teach-me.md](../../playbook/teach-me.md)
  本库写课覆盖层。Use for: 文件放哪、共用 chrome、导航、配图、标题、`/stop-slop`。
- [playbook/teach 课件：嵌入权威中文](../../playbook/teach-课件-嵌入权威中文.md)
  精读课分支。Use for: 课里点名的 skill / prompt 要把中文嵌进 HTML。

## Wisdom (Communities)

- [mattpocock/skills Issues](https://github.com/mattpocock/skills/issues)
  上游真实踩坑。Use for: 文件写进 skill 目录 ([#377](https://github.com/mattpocock/skills/issues/377))、测验正确答案总在第一项 ([#335](https://github.com/mattpocock/skills/issues/335))、没有摸底步骤 ([#725](https://github.com/mattpocock/skills/issues/725))。
- [aihero.dev/skills](https://www.aihero.dev/skills)
  作者的人读 docs 站。Use for: 和仓库 `docs/` 对照。

## Gaps

- skill 要求间隔和交错，但没有复习日程或 Anki 接线。
- 第一课没有正式摸底；起始水平靠你自己说，或靠空的 `learning-records/`。
- `GLOSSARY-FORMAT.md` 还在 skill 包里，`SKILL.md` 已经不链它（[#559](https://github.com/mattpocock/skills/issues/559)）。
