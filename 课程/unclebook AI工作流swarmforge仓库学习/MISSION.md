...
你已经在用单 agent + skills 的编排（例如 `mattpocock/skills`），现在要读懂 Uncle Bob 的 SwarmForge：怎么用 **多角色、多 worktree、tmux + 仪表盘** 把一群 agent 管成可交接的软件工程流水线。学完后，你能对照自己的 AI 开发流程，判断哪些纪律（角色切分、handoff、constitution）值得搬过来，而不是把整套 tmux 编排原样抄进日常。

## Success looks like

- 能画出 `main` 与 pack 分支的分工：共享脚本 / 共享宪法 vs 可运行工作流。
- 能解释一次任务如何从 New Task 走到 Done：master agent、git_handoff、人类闸门。
- 能对照 `two-pack` / `four-pack` / `six-pack` 说出各自适用的任务规模，而不是默认上满六角色。
- 能指认 `swarmforge/` 里三样东西各自管什么：constitution 文章、handoff 协议、scripts。

## Constraints

- 课程包放在本知识库的 `课程/` 下，课件用 HTML，源码引用走 GitHub，保证换机器仍能打开。
- 先建立地图，再把一条 pack **整台走完**；不把交接协议拆成单独一课。第一课不是安装课。
