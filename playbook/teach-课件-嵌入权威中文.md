...
2. **嵌入中文，原文只链**  
   - 正文放可复用块（建议 `article.prompt-doc` + `pre.prompt-zh`，样式进 `课程/assets/course.css`）。  
...
4. 复用 `课程/assets/course.css`；缺嵌入用样式则补进这份共用 CSS，不要在该课 `assets/` 再复制一份。先读 `MISSION` / `NOTES` / `RESOURCES` 与上游原文，勿编造全文。
