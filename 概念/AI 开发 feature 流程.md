...
这篇不是在定义唯一正确的方法，而是在记录一种我目前觉得很顺的 AI 开发 feature 流程。它更像一个概念框架：把“从想法到落地”拆成几个稳定阶段，并且在每一阶段都尽量留下可复用的外部产物。

![AI 开发 feature 流程示意图](../assets/program_process.png)
...
这也让它和前面仓库里那几篇概念能接起来：agent 想稳定，最终还是要靠 environment，而不是只靠某次对话状态。

## 1. Grill：先把模糊想法问清楚

起点通常不是完整需求，而只是一个模糊念头：一个 app、一个 feature、一次重构、一个“也许可以这样”的方向。

`Grill` 阶段的目标不是立刻开工，而是先把关键分支问出来，例如：
...
这篇先放在 `概念/`，因为我现在更想把它记成“理解 AI feature 开发为什么要分阶段”的概念模型，而不是已经完全验证好的个人标准作业程序。

如果后面你在真实项目里反复用它，而且每一阶段的进入条件、退出条件、产物模板都稳定了，再把它提炼进 `playbook/` 会更合适。

## 对我的启发

- AI 开发的关键不是只让 agent 快写代码，而是让中间产物持续留下来。
- 真正稳定的 feature 流程，核心是 `Grill -> Spec -> Tickets -> Implement -> Review` 这条链不断把模糊性往外部文档和工作单元里挪。
