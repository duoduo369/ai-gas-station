...
- 先确认人已经明确判断：这条候选 skill 已经过真实使用验证，现在可以作为标准入口推荐；这一步必须由人拍板，不要自己代判。
...
3. 不通过就报告并停止。
   完成标准：只要人类判断未通过，或自包含检查没过，就已经明确说明缺口，并保持目录和生命周期入口原样不动；不要一边 release 一边顺手修。
4. 通过后再执行 release。
   完成标准：目标目录已经从 `skills/in-process/<skill-slug>/` 提升到 `skills/stable/<skill-slug>/`，相关 README 入口也已同步更新，不再同时留着一份过期的候选入口。
...
- 确认成功路径里，skill 本体与相关 README 入口都已经和新的生命周期桶一致。
